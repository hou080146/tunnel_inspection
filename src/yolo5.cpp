...
bool Yolov5ORT::init(const std::string modelPath, const bool isGPU = true, const cv::Size inputSize = cv::Size(640, 640)) {
    ...
    */
    ...
    auto inputName = std::move(session.GetInputNameAllocated(0, allocator));
    //inputNames.push_back(inputName.get());
    inputNames.push_back("images");
    auto outputName = std::move(session.GetOutputNameAllocated(0, allocator));
   // outputNames.push_back(outputName.get());
    outputNames.push_back("output0");

    auto type_info_output0 = session.GetOutputTypeInfo(0);

    auto tensor_info_output0 = type_info_output0.GetTensorTypeAndShapeInfo();
    auto outputNodeDataType = tensor_info_output0.GetElementType();
    auto outputTensorShape = tensor_info_output0.GetShape();
    auto tensor_info_output1 = type_info_output0.GetTensorTypeAndShapeInfo();
...
std::vector<Detection> Yolov5ORT::detect(cv::Mat& image, const float confThreshold = 0.4, const float iouThreshold = 0.45) {
    ...
    */
    ...
    //std::vector<int64_t> inputTensorShape = session.GetInputTypeInfo(0).GetTensorTypeAndShapeInfo().GetShape();
