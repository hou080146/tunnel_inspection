...
public:
	...
	void saveAs(QString& path);				//新建word另存为
	...
	void setStrErrorInfo(const QString& errorInfo);
	...
	void setLineSpacing(int flag);                      //设置行距
	void setRowAlignment(/*int nTable*/QAxObject* table, int row, int flag);
	//同时设置列宽和列高可以固定图片的大小
	...
	QAxObject* intsertTable(int row, int column);							//插入一个几行几列表格
	void insertMoveDown();											//插入回车
	void insertText(const QString& text);							//插入文字
	void InsertBreak();                                            //插入分页符
	void insertTable(int tableIndex, int row, int column);
    void insertHyperlink(const QString& filePath, const QString& displayText);
	//插入巡检单位  巡检计划 巡检时间 巡 检 人  顺序传参
	//void insertTitle(const QString& str1,const QString& str2,const QString& str3,const QString& str4);
	void MergeCells(/*int tableIndex*/QAxObject* table, int nStartRow, int nStartCol, int nEndRow, int nEndCol);//合并单元格
	bool open(const QString& strFilePath, bool bVisable);
	bool openword(bool bVisable);
	bool isOpen();
	void setFirstLineIndent(int indent);
	
	void setColumnWidth(int nTable, int column, int width);////设置表格列宽
	void setCellString(/*int nTable*/QAxObject* table, int row, int column, const QString& text);
	void insertCellPic(/*int nTable*/QAxObject* table, int row, int column, const QString& picPath);  //在表格中插入图片
	void setCellFontBold(int nTable, int row, int column, bool isBold); //设置内容粗体
	void setCellFontSize(int nTable, int row, int column, int size); ////设置文字大小
	//void addTableRow(int nTable ,int nRow,int rowCount);
	void addTableRow(int tableIndex, int nRow, int rowCount);  
	void setColumnHeight(int nTable, int column, int height);  ////设置表格列宽
